...
def load_weights(model, wt_file):
        ...
        start = load_conv_bn(buf, 5, model.darknet.cnn[0].conv,
                             model.darknet.cnn[0].bn)
        n = [2,3,9,9,5]
        for i in range(1,6):
            start = load_conv_bn(buf,
                                 start,
                                 model.darknet.cnn[i][0].conv,
                                 model.darknet.cnn[i][0].bn)
            for j in range(1, n[i-1]):
                for k in range(2):
                    start = load_conv_bn(buf,
                                         start,
                                         model.darknet.cnn[i][j].conv[k].conv,
                                         model.darknet.cnn[i][j].conv[k].bn
                                         )
